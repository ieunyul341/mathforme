...
const DAY_CHECK_KEYS = ["mathLecture", "conceptMath", "maplMath", "scienceLecture", "o2Science", "wanjaScience", "output"];
const BOOK_CHECK_KEYS = ["conceptMath", "maplMath", "o2Science", "wanjaScience"];
...
function defaultDayState() {
  return {
    ...
    logs: { conceptMath: "", maplMath: "", o2Science: "", wanjaScience: "" },
...
function getDayBudget(day) {
  ...
  return school
    ? { math: state.timeBudgets.schoolMath, science: state.timeBudgets.schoolScience, school: state.timeBudgets.schoolReview, mode: "개학 후" }
    : { math: state.timeBudgets.vacationMath, science: state.timeBudgets.vacationScience, school: 0, mode: "방학" };
}
...
function syncSettingsInputs() {
  ...
  $("#schedule-note").textContent = `${formatDate(state.startDate)} 시작 · ${formatDate(getStudyEndDateString())} 본학습 종료 · 방학 모드 ${vacationDays}일 / 개학 후 모드 ${30 - vacationDays}일`;
}
...
function renderCalendar() {
  ...
  $("#calendar-agenda").innerHTML = STUDY_PLAN.map(item => {
    const dateString = getDateStringForDay(item.day);
    const reviews = getPlannedReviewEventsOnDate(dateString);
    return `<button class="agenda-row" type="button" data-open-day="${item.day}">
      <div class="agenda-date"><strong>Day ${item.day}</strong><span>${formatDate(dateString)}</span></div>
...
function openDay(day) {
  ...
  $("#day-budget-banner").innerHTML = `<div><strong>${budget.mode} 시간표</strong><span>수학 ${formatMinutes(budget.math)} · 과학 ${formatMinutes(budget.science)}${budget.school ? ` · 학교복습 ${formatMinutes(budget.school)}` : ""}</span></div><div class="fit-badges"><span class="${mathFit ? "fit" : "over"}">수학 ${mathFit ? "적합" : "초과"}</span><span class="${scienceFit ? "fit" : "over"}">과학 ${scienceFit ? "적합" : "초과"}</span></div>`;
  ...
  $("#output-prompt").textContent = item.outputPrompt;
...
function printDay(day) {
  const item = getPlan(day);
  const d = getDayState(day);
  const title = d.resultTitle || `Day ${day} 공통수학2·통합과학2 한 장 정리`;
...
function updateTimeBudget(key, hours) {
...
function bindEvents() {
  ...
  $("#complete-day-button").addEventListener("click", () => {
    if (!activeDay) return;
    ["mathLecture", "conceptMath", "maplMath", "scienceLecture", "o2Science", "wanjaScience"].forEach(key => setTaskDone(activeDay, key, true));
